...
def FrameCapture(path):
    ...
    while success:
       ...
       # im_resized = cv2.resize(image, (224, 224), interpolation=cv2.INTER_LINEAR)
        ...
        crop(frameoriginal, (250, 200, 600, 400), "frameASHWAMEDHA%d.jpg" % count)
       # cropped_image.save(saved_location)
        #cropped_image.show()
       ...
       # print(colors)
        for x in colors:
            ...
            print(x[0][1])
            print(x[0][2])"""
            if ((x[0][0] == 223 and x[0][1] == 239 and x[0][2] == 140) or (
                    x[0][0] == 221 and x[0][1] == 236 and x[0][2] == 141) or count==126):
                print('alert there')
                nvar="8"
                firebase.database().child("RealTime Node").child("Mane").set(nvar);
